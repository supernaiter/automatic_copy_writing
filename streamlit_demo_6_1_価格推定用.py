...
def get_model_categories() -> Dict[str, Dict]:
    ...
    return {
        "🎯 標準GPTモデル（推奨）": {
            ...
            "gpt-4.1": {
                "price_string": "$2.00 / $8.00",
                "price": {"input_per_million": 2.00, "output_per_million": 8.00},
...
# コスト情報表示
st.sidebar.header("💰 コスト情報")
if 'execution_results' in st.session_state and st.session_state.execution_results:
    total_cost = sum(execution.get('cost_info', {}).get('cost_usd', 0) for execution in st.session_state.execution_results)
    total_tokens = sum(execution.get('cost_info', {}).get('total_tokens', 0) for execution in st.session_state.execution_results)
    
    st.sidebar.metric("累計コスト", f"${total_cost:.4f}")
    st.sidebar.metric("累計トークン", f"{total_tokens:,}")
    
    # 最新の実行結果のコスト情報
    latest_execution = st.session_state.execution_results[-1]
    latest_cost_info = latest_execution.get('cost_info', {})
    if latest_cost_info:
        st.sidebar.info(f"最新実行: ${latest_cost_info.get('cost_usd', 0):.4f}")
elif 'batch_results' in st.session_state and st.session_state.batch_results:
    total_cost = sum(execution.get('cost_info', {}).get('cost_usd', 0) for execution in st.session_state.batch_results)
    total_tokens = sum(execution.get('cost_info', {}).get('total_tokens', 0) for execution in st.session_state.batch_results)
    ...
    latest_cost_info = latest_batch.get('cost_info', {})
    if latest_cost_info:
        st.sidebar.info(f"最新実行: ${latest_cost_info.get('cost_usd', 0):.4f}")
else:
    st.sidebar.info("まだ実行されていません")
