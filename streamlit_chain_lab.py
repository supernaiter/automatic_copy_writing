...
#!/usr/bin/env python3
"""Copy Chain Lab - experiment with chained copy generation."""
import json
import os
import textwrap
import time
...
BASE_SYSTEM_MESSAGE = textwrap.dedent(
    """You are a senior Japanese copywriter. Review the given context and past conversation to craft memorable advertising copy.
...
def generate_copies(
    ...
    messages = build_messages(orientation, prompt, copy_count, conversation_messages)
...
def main() -> None:
    st.set_page_config(page_title="Copy Chain Lab", layout="wide")
    st.title("Copy Chain Lab")
    st.caption("前回の生成結果を踏まえて連続的にコピーを磨く実験用インターフェース")
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        if st.button("チェイン履歴をクリア", type="secondary"):
            st.session_state.history = []
            st.session_state.conversation_messages = []
            st.info("チェイン履歴をクリアしました。")
    ...
    if generate:
        if not prompt_text.strip():
            ...
            st.download_button(
                ...
                file_name="copy_chain_generation.txt",
                mime="text/plain",
            )
            ...
            # 最新のユーザーメッセージは最後に追加されたもの
            latest_user_message = messages[-1]
            st.session_state.conversation_messages.append(latest_user_message)
            assistant_content = "\n".join(
                f"{idx+1}. {copy}" for idx, copy in enumerate(copies)
            ) if copies else raw_text
